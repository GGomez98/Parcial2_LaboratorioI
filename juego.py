...
def crear_invasores(cantidad, lista_invasores, velocidad_misil_min, velocidad_misil_max):

    for _ in range(cantidad):
        invasor = objetos.Invasor(70,50,(0,255,0), 50, velocidad_misil_min, velocidad_misil_max)
...
while running:
    ...
    if pantalla_actual == 4:
        if not nivel_iniciado:
            crear_invasores(5, invasores, 5, 10)
            nivel_iniciado = True
        elif len(invasores) == 0 and nivel_iniciado:
            pantalla_actual = 5
            nivel_iniciado = False
        puntaje = nivel_1(window, invasores, nave, timer_10_milisegundos, timer_recuperacion, timer_segundo, timer_disparo_invasores, tiempo, fuente_2, puntaje)
    ...
    if pantalla_actual == 6:
        if not nivel_iniciado:
            crear_invasores(10, invasores, 10, 15)
            nivel_iniciado = True
        elif len(invasores) == 0 and nivel_iniciado:
            pantalla_actual = 7
            nivel_iniciado = False
    ...
    if pantalla_actual == 8:
        if not nivel_iniciado:
            crear_invasores(20, invasores, 20, 25)
            nivel_iniciado = True
        puntaje = nivel_1(window, invasores, nave, timer_10_milisegundos, timer_recuperacion, timer_segundo, timer_disparo_invasores, tiempo, fuente_2, puntaje)
